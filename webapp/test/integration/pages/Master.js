		...
		onTheMasterPage: {
			...
			actions: {
									...
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("Codiexp") === oObject.Codiexp;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].Codiexp;
									} else {
										sCurrentId = aItemsNotInTheList[0].Codiexp;
									}
				...
				iRememberTheListItem: function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("Codiexp"),
						title: oBindingContext.getProperty("Objecte")
					};
				},
			...
			assertions: {
				...
				theListHeaderDisplaysZeroHits: function () {
					return this.waitFor({
						...
						matchers: new PropertyStrictEquals({
							name: "title",
							value: "ExpedientSet (0)"
						}),
						success: function () {
							Opa5.assert.ok(true, "The list header displays 'ExpedientSet (0)'");
						},
				...
				theListShouldBeSortedAscendingOnName: function () {
					return this.theListShouldBeSortedAscendingOnField("Objecte");
				},
				...
				theListShouldHaveAllEntries: function () {
					...
					// retrieve all ExpedientSet to be able to check for the total amount
					...
					return this.waitFor({
						...
						success: function (oList) {
							...
							this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "title",
									value: "ExpedientSet (" + iExpectedLength + ")"
								}),
								success: function () {
